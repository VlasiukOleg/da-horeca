  ...
  <section class="py-16 md:py-24 bg-gray-100 dark:bg-gray-900/20">
    <UContainer>
      <div class="text-center mb-16">
        <h2 class="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">{{ data?.title || 'Наші партнери та екосистема' }}</h2>
      ...
      <div class="flex flex-wrap justify-center gap-6 max-w-5xl mx-auto">
        <div v-for="(partner, index) in data?.partners" :key="index" class="w-full lg:w-[calc(50%-0.75rem)] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-6 rounded-2xl shadow-sm hover:shadow-md hover:border-brand-300 dark:hover:border-brand-700 transition-all duration-300 group">
